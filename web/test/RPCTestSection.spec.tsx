...
describe("RPCTestSection Component", () => {
  describe("With Subsystem", () => {
    it("should render RPC controls when subsystem is found", () => {
      ...
      expect(
        screen.getByText(/Query the firmware for the currently active layer/i)
      ).toBeInTheDocument();
      expect(screen.getByText(/Get Active Layer/i)).toBeInTheDocument();
    });
  });
  ...
  describe("Without Subsystem", () => {
    it("should show warning when subsystem is not found", () => {
      ...
      expect(
        screen.getByText(/Subsystem "nano0nfire__active_layer_notify" not found/i)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          /Make sure your firmware includes the active_layer_notify module/i
        )
      ).toBeInTheDocument();
    });
  });
