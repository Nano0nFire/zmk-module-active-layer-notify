...
TEST_BUILD_DIR_NAME = "tests-zmk-module-active_layer_notify-with-custom-studio-rpc"
...
class WestCommandsTests(unittest.TestCase):
    ...
    def test_zmk_build(self):
        self._test_zmk_build(
            {
                "module_active_layer_notify_board_feature_disabled": ConfigAndDeviceTree(
                    config=[
                        'CONFIG_ZMK_KEYBOARD_NAME="Module Test"',
                        "CONFIG_ZMK_USB=y",
                        "CONFIG_ZMK_BLE=y",
                        "# CONFIG_ZMK_ACTIVE_LAYER_NOTIFY is not set",
                    ],
                    device=[
                        "DT_COMPAT_HAS_OKAY_zmk_keymap",
                    ],
                ),
                "module_active_layer_notify_board_with_rpc": ConfigAndDeviceTree(
                    config=[
                        "CONFIG_ZMK_STUDIO=y",
                ...
                "module_active_layer_notify_board_without_rpc": ConfigAndDeviceTree(
                    config=[
                        "CONFIG_ZMK_ACTIVE_LAYER_NOTIFY=y",
                        "# CONFIG_ZMK_STUDIO is not set",
                        NotFound("CONFIG_ZMK_ACTIVE_LAYER_NOTIFY_STUDIO_RPC"),
                    ],
                    device=[],
                ),
